...
import type { SalesInvoice, SalesInvoiceItem, CompanySettings } from '@prisma/client';

// ── Types ─────────────────────────────────────────────────

type InvoiceWithItems = SalesInvoice & { items: SalesInvoiceItem[] };
...
export async function generateInvoicePdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    const snap     = invoice.customer_snapshot as Record<string, any>;
    const currency = snap.currency ?? 'INR';
    const country  = snap.country  ?? 'IN';
    ...
    const statusColors: Record<string, string> = {
      draft: '#64748b', sent: '#1d4ed8', paid: '#15803d', void: '#dc2626',
    };
    ...
    doc.fontSize(10).font('Helvetica-Bold').fillColor('#1e293b')
       .text(snap.customer_name ?? snap.company_name ?? '—', 48, billBoxY + 20);
    doc.fontSize(8).font('Helvetica').fillColor('#64748b');
    let btY = billBoxY + 34;
    ...
    if (invoice.shipping_address) {
      doc.fontSize(9).font('Helvetica-Bold').fillColor('#1e293b')
         .text(snap.customer_name ?? '—', shipX, billBoxY + 20);
      doc.fontSize(8).font('Helvetica').fillColor('#64748b')
    ...
    const colW = { num: 20, desc: 150, hsn: 60, qty: 35, price: 70, tax: 35, total: 75 };
    const cols = {
      num:   40,
      desc:  40 + colW.num,
      hsn:   40 + colW.num + colW.desc,
      qty:   40 + colW.num + colW.desc + colW.hsn,
      price: 40 + colW.num + colW.desc + colW.hsn + colW.qty,
      tax:   40 + colW.num + colW.desc + colW.hsn + colW.qty + colW.price,
      total: 40 + colW.num + colW.desc + colW.hsn + colW.qty + colW.price + colW.tax,
    };
    ...
    doc.text('HSN/SAC',     cols.hsn,   tableY + 7, { width: colW.hsn });
    doc.text('Qty',         cols.qty,   tableY + 7, { width: colW.qty,   align: 'right' });
    doc.text('Unit Price',  cols.price, tableY + 7, { width: colW.price, align: 'right' });
    doc.text('Tax%',        cols.tax,   tableY + 7, { width: colW.tax,   align: 'right' });
    doc.text('Total',       cols.total, tableY + 7, { width: colW.total, align: 'right' });
    ...
    invoice.items.forEach((item, i) => {
      ...
      doc.text(item.description,            cols.desc,  rowY + 7, { width: colW.desc });
      doc.text(item.hsn_sac ?? '—',        cols.hsn,   rowY + 7, { width: colW.hsn });
      doc.fillColor('#64748b')
         .text(String(Number(item.quantity)),  cols.qty,   rowY + 7, { width: colW.qty,   align: 'right' })
         .text(fmt(Number(item.unit_price)),   cols.price, rowY + 7, { width: colW.price, align: 'right' })
         .text(`${totalTaxPct}%`,              cols.tax,   rowY + 7, { width: colW.tax,   align: 'right' });
      doc.font('Helvetica-Bold').fillColor('#1e293b')
         .text(fmt(Number(item.line_total)),   cols.total, rowY + 7, { width: colW.total, align: 'right' });
    ...
    doc.text(fmt(Number(invoice.subtotal)), totX + totW - 60, tY, { width: 60, align: 'right' });
    tY += 14;

    if (Number(invoice.discount_amount) > 0) {
      doc.text(`Discount (${Number(invoice.discount_percent)}%)`, totX, tY, { width: totW - 60 });
      doc.text(`-${fmt(Number(invoice.discount_amount))}`, totX + totW - 60, tY, { width: 60, align: 'right' });
      tY += 14;
    }
    ...
    doc.text(fmt(Number(invoice.total)), totX + totW - 60, tY, { width: 60, align: 'right' });
    ...
    doc.fontSize(8).font('Helvetica').fillColor('#1e293b')
       .text(numberToWords(Number(invoice.total)), 48, wordsY + 14, { width: PAGE_WIDTH - 16 });
    ...
    if (invoice.payment_terms) {
      const ptY = wordsY + 130;
      doc.fontSize(7).font('Helvetica-Bold').fillColor('#94a3b8')
         .text('PAYMENT TERMS', 40, ptY);
      doc.fontSize(8).font('Helvetica').fillColor('#475569')
         .text(invoice.payment_terms, 40, ptY + 12, { width: PAGE_WIDTH });
    }
