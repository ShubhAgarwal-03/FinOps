...
import { paymentsService } from '../../lib/api/ar/index';
import type { Payment, PaymentMethod } from '@/types/ar';
import type { Invoice } from '@/services/ar';
import { formatCurrency } from '../../../../libs/shared/utils/currency.utils';
import { formatDate } from '../../../../libs/shared/utils/date.utils';

// PLACE AT: apps/web/components/ar/paymentPanel.tsx
// Ported from the original Invoice-Generator's components/invoices/PaymentPanel.tsx,
// re-wired to FinOps: paymentsService from lib/api/ar (POST/GET
// /api/ar/invoices/:id/payments), Payment/PaymentMethod from @/types/ar,
// and the Invoice shape from services/ar.ts (the same one the detail page
// uses). Embedded inline on the invoice detail page rather than living on
// its own route.
...
export default function PaymentPanel({ invoice, onInvoiceUpdate }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (amount > balanceDue + 0.1) {
      toast.error(`Amount exceeds balance due (${fmt(balanceDue)})`);
      return;
    }
    setSubmitting(true);
    try {
      const { payment, invoice: updatedInvoice } = await paymentsService.record(invoice.id, {
        amount,
        method: form.method,
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        ...
        {paymentStatus !== 'paid' && invoice.status !== 'void' && (
          <button
