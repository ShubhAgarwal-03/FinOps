...
//
// This file consolidates every AP frontend client. Each section below has
// been checked against the actual Express routes it calls — see inline
// notes wherever a call was corrected to match what the backend really
// exposes (method, path, or field name).
// ─────────────────────────────────────────────────────────────────────────────
...
export const rfpService = {
  ...
  // submitVendorQuote() upserts by (rfp_id, vendor_id) server-side, so call
  // addQuote again with the same vendor_id to revise a quote.
  addQuote: (rfp_id: string, data: Partial<VendorQuote>) =>
    apiClient.post<VendorQuote>(`/api/ap/rfp/${rfp_id}/quotes`, data).then(r => r.data),
...
export const disputesService = {
  ...
  getForInvoice: (vendor_invoice_id: string) =>
    apiClient.get<DisputeRecord[]>(`/api/ap/disputes?${buildParams({ vendor_invoice_id })}`).then(r => r.data),
  create: (data: {
